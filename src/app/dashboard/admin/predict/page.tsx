...
export default function AdminPredictPage() {
  ...
  // Load students for the autofill helper
  useEffect(() => {
    const fetchStudentsForAutofill = async () => {
      try {
        const res = await axios.get('/api/students?limit=20');
        if (res.data.success) {
          setStudents(res.data.students);
        }
      } catch (err) {
        console.error('Error loading autofill student options:', err);
      }
    };
    fetchForwards();
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <h2 className="text-xl font-extrabold tracking-tight flex items-center gap-2">
              <BrainCircuit className="h-6 w-6 text-indigo-500" /> AI Student Risk Predictor
            </h2>
            <p className="text-xs text-slate-400 font-semibold leading-relaxed">
              Feed academic performance variables to calculate expected passing probabilities, backlog risks, and detailed suggestions.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Total lecture attendances (0 to 100).</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Average self-study duration per day (0 to 24).</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Mid-term internal quizzes sum.</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Cumulative classroom assignments.</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Laboratory file entries and oral assessments.</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Uncleared backlog subjects from previous terms.</span>
            </motion.div>
            ...
            <motion.div whileTap={{ scale: 0.99 }} className="space-y-1.5 md:col-span-2">
              ...
              <input
                ...
                required
              />
              <span className="text-[9px] text-slate-500 block">Grade Point Average scored in immediate previous term.</span>
            </motion.div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>Execute Diagnostic AI</span>
                </>
              )}
            </motion.button>
          </div>
      ...
      {/* AI Prediction Result popup modal */}
      <PredictionModal
